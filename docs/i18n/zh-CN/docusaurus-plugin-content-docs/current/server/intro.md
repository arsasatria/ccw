...
Claude Code Wrapper Server 是一个核心服务组件，负责将 Claude Code 的 API 请求路由到不同的 LLM 提供商。它提供了完整的 HTTP API，支持：
...
CCW server 通过以下方式集成 `@musistudio/llms`：
...
`@musistudio/llms` 的当前版本是 `1.0.51`。它作为独立的 npm 包发布，可以独立使用或作为 CCW Server 的一部分使用。
...
```bash
docker run -d -p 3456:3456 musistudio/ccw
```
