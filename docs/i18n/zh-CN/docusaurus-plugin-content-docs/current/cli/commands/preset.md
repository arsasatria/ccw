...
**示例：**
```bash
ccw preset export my-config --description "我的生产环境配置" --author "您的名字"
```

**执行过程：**
1. 读取 `~/.ccw/config.json` 中的当前配置
2. 提示输入描述、作者和关键字（如未通过命令行提供）
3. 自动清理敏感字段（API 密钥变为占位符）
4. 在 `~/.ccw/presets/<名称>/` 创建预设目录
...
```bash
ccw preset delete <名称>
ccw preset rm <名称>
ccw preset remove <名称>
```
