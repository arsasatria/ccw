...
CLI uses the same configuration file as Server: `~/.ccw/config.json`
...
Edit `~/.ccw/config.json` with your favorite editor:

```bash
nano ~/.ccw/config.json
```
...
```
~/.ccw/config.backup.{timestamp}.json
```
