...
mkdir -p ~/.ccw/presets/simple-openai

# Create manifest.json
cat > ~/.ccw/presets/simple-openai/manifest.json << 'EOF'
{
  "name": "simple-openai",
  "version": "1.0.0",
...
```bash
# Create preset directory
mkdir -p ~/.ccw/presets/advanced-config

# Create manifest.json
cat > ~/.ccw/presets/advanced-config/manifest.json << 'EOF'
{
...
- **CLI Mode**: Place directory in `~/.ccw/presets/`, then run `ccr preset install preset-name`
