...
async function listPresets(): Promise<void> {
  ...
  try {
    ...
    console.log(`\nCreate your first preset with: ${GREEN}ccw preset export <name>${RESET}\n`);
    return;
  }
  ...
  if (presetDirs.length === 0) {
    console.log('\nNo presets found.');
    console.log(`\nInstall a preset with: ${GREEN}ccw preset install <file>${RESET}\n`);
    return;
  }
  ...
  switch (subCommand) {
    case 'export':
      ...
      if (!presetName) {
        ...
        console.error('Usage: ccw preset export <name> [--output <path>] [--description <text>] [--author <name>] [--tags <tags>]\n');
        process.exit(1);
      }
    ...
    case 'install':
      ...
      if (!source) {
        ...
        console.error('Usage: ccw preset install <file | url | name>\n');
        process.exit(1);
      }
    ...
    case 'remove':
      ...
      if (!deleteName) {
        console.error('\nError: Preset name is required\n');
        console.error('Usage: ccw preset delete <name>\n');
        process.exit(1);
      }
    ...
    case 'info':
      ...
      if (!infoName) {
        console.error('\nError: Preset name is required\n');
        console.error('Usage: ccw preset info <name>\n');
        process.exit(1);
      }
    ...
    default:
      ...
      console.error('  ccw preset export <name>      Export current configuration as a preset');
      console.error('  ccw preset install <source>   Install a preset from file, URL, or registry');
      console.error('  ccw preset list              List installed presets');
      console.error('  ccw preset info <name>        Show preset information');
      console.error('  ccw preset delete <name>      Delete a preset\n');
      process.exit(1);
  }
}
