...
export async function installPresetFromMarket(presetName: string): Promise<{ name: string; preset: PresetFile }> {
  ...
  if (!marketPreset) {
    throw new Error(
      ...
      `Please check the available presets at: https://github.com/ccw/presets`
    );
  }
  ...
  if (await isPresetInstalled(installedPresetName)) {
    throw new Error(
      ...
      `To delete and reinstall, use: ccw preset delete ${installedPresetName}\n` +
      `To reconfigure without deleting, use: ccw preset install ${installedPresetName}`
    );
  }
